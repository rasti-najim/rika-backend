...
const { appendFilesToFile, readFileContentsAsync, readFileContentsSync, } = require("../functions/core_memory");
...
router.post("/", (req, res) => __awaiter(this, void 0, void 0, function* () {
    try {
        ...
        let prevMessages = (yield loadMessages(messagesFile)).map(({ message }) => message);
        ...
        yield saveMessages(newMessages, messagesFile);
        const useTools = completion.choices[0].finish_reason === "tool_calls";
        if (useTools) {
            const toolMessage = yield handleToolCalls(completion, messages);
...
function handleToolCalls(completion, messages) {
    return __awaiter(this, void 0, void 0, function* () {
        console.log("calling handleToolCalls");
        const arguments = JSON.parse(completion.choices[0].message.tool_calls[0].function.arguments);
        const tool_call_id = completion.choices[0].message.tool_calls[0].id;
        const toolMessage = {
            role: "tool",
        ...
        if (completion.choices[0].message.tool_calls[0].function.name ===
        ...
        else if (completion.choices[0].message.tool_calls[0].function.name ===
            "core_memory_replace") {
            coreMemoryReplace(arguments.name, arguments.old_content, arguments.new_content);
        }
        else if (completion.choices[0].message.tool_calls[0].function.name ===
            "conversation_search") {
            console.log("calling conversation_search", arguments.query);
            const searchResult = yield conversationSearch(arguments.query);
            console.log(searchResult);
            toolMessage.content = searchResult;
        }
        else if (completion.choices[0].message.tool_calls[0].function.name ===
            "conversation_search_date") {
            console.log("calling conversation_search_date", arguments.start_date, arguments.end_date);
            const searchResult = yield conversationSearchDate(arguments.start_date, arguments.end_date);
            console.log(searchResult);
            toolMessage.content = searchResult;
        }
        else if (completion.choices[0].message.tool_calls[0].function.name ===
            ...
            yield archivalMemoryInsert(arguments.content, openai);
        }
        else if (completion.choices[0].message.tool_calls[0].function.name ===
            "archival_memory_search") {
            console.log("calling archival_memory_search", arguments.query);
            yield archivalMemorySearch(arguments.query, openai);
        }
        else if (completion.choices[0].message.tool_calls[0].function.name ===
            "archival_memory_delete") {
            yield client.deleteCollection({ name: "archival_memory" });
        }
        return toolMessage;
        // try {
