 ...
 */
async function textSearch(queryString, count = null, start = null) {
  ...
  const matches = messagePool.filter(
    (d) =>
      d.message.content &&
      d.message.content.toLowerCase().includes(queryString.toLowerCase())
  );
 ...
 */
async function conversationSearch(query, page = 0) {
  ...
  const [results, total] = await textSearch(query, count, page * count);
